...
import ru.habit.tracker.ui.theme.Typography

@Composable
fun AnimatedCircularProgressIndicatorWithText(
    currentValue: Int,
    maxValue: Int,
    ...
    val stroke = with(LocalDensity.current) {
        Stroke(width = 8.dp.toPx(), cap = StrokeCap.Round, join = StrokeJoin.Round)
    }
    ...
    Text(modifier = modifier, text = buildAnnotatedString {
        val emphasisSpan =
            Typography.titleLarge.copy(color = if (currentValue == maxValue) completedColor else progressIndicatorColor)
                .toSpanStyle()
        val defaultSpan = Typography.bodyMedium.copy(
            color = progressBackgroundColor, fontWeight = FontWeight.Bold, fontSize = 17.sp
        ).toSpanStyle()
        append(AnnotatedString("$currentValue", spanStyle = defaultSpan))
...
private val CircularIndicatorDiameter = 124.dp
